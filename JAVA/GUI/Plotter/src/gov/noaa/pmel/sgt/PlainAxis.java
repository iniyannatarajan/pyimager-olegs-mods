/*
 * $Id: PlainAxis.java 8825 2006-06-28 14:44:59Z pompert $
 *
  ...
  void draw(Graphics g) {
    ...
    if(orientation_ == Axis.HORIZONTAL) {
      ...
      if(dir*uRange_.start <= 0 && dir*uRange_.end >= 0) {
        x = (Math.floor(uRange_.start/(delta*labelInterval_) - 0.00001))*delta*labelInterval_;
      } else {
        x = xt;
      }
      ...
      for(i=0; i <= istop; i++) {
        ...
        //labelText = DecimalFormat.getInstance().format(x);
        //labelText = String.format("%1$g",x);
      ...
      //
      dir = delta > 0? 1.0: -1.0;
      yt = Math.floor((uRange_.start/delta) + (dir*uRange_.start > 0? 1.0: -1.0)*0.00001)*delta;
      ...
      if(dir*uRange_.start <= 0 && dir*uRange_.end >= 0) {
        y = (Math.floor(uRange_.start/(delta*labelInterval_) - 0.00001))*delta*labelInterval_;
      } else {
        y = yt;
      }
      ...
      for(i=0; i <= istop; i++) {
        ...
        //labelText = DecimalFormat.getInstance().format(y);
        //labelText = String.format("%1$g",y);
        label = new SGLabel("coordinate", labelText, new Point2D.Double(xt, yt));
