...
/**
 * Base TemplateTreeManager Class
 *
 * @created 13-07-2006, 16:00
 *
 * @author pompert
 *
 * @version $Id: TemplateTreeManager.java 22364 2012-10-16 14:41:04Z coolen $
 *
 * @updated
 */
public class TemplateTreeManager extends GenericTreeManager implements ITreeManager{
    
    // Create a Log4J logger instance
    private static Logger logger = Logger.getLogger(TemplateTreeManager.class);
    private static TemplateTreeManager instance;
    
    /**
     * default constructor, protected by a singleton pattern
     */
    private TemplateTreeManager(UserAccount anAccount) {
        super(anAccount);
    }
    
    public static TemplateTreeManager getInstance(UserAccount anAccount){
        if(instance==null){
            instance = new TemplateTreeManager(anAccount);
        }
        return instance;
    }
    ...
    public void defineChildsForNode(TreeNode aNode) {
        logger.trace("Entry - TemplateTreeManager defineChildNodes("+toString()+")");
